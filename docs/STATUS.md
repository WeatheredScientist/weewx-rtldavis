...
1. **Promote v2.0.4** — merge `dev` → `main`, tag `v2.0.4` + `prod-baseline-2026MMDD`, **push the image
   to Docker Hub**, cut the GitHub release. This is the top item and it has grown: the published image
   ships both the **stock driver** (DEC-0031) *and* the **console-handler freeze hazard** (DEC-0036).
   Neither is fixed for anyone downstream until the image lands.
2. **The upstream post** — owner call. The research is done (DEC-0035). What is left is prose: rewrite it
   in his voice, balance technical detail with human warmth, and **do not post without an explicit go.**
3. **Cross-project harmonization** — `docs/handoffs/S37-to-all-projects-stdout-freeze.md` is written and
   goes to the dashboard + HLF (**advisory only; we changed nothing in their repos**). It carries the
   owner's open architectural question: three shared assets (the NAS Docker daemon, the driver-vs-image
   mismatch, the secret gate) have each now caused a cross-repo incident, and **none of them belongs to
   any one repo**. Options are laid out; the call is his.
4. **The five secret-gate holes** the dashboard found (their S70 §3) — our repo is public, so these matter
   more here. Their `scripts/test_check_secrets.sh` harness (plant known-bad payloads that MUST be
   caught) is the method to steal. **Do not port their anchor verbatim** — theirs runs on raw lines, ours
   on `grep -n` output; a verbatim port would silently re-open the hole.
5. Then: cold-load Fix B (`current.json`), the temp/humidity coupling filter, Reception Layer B (v2.0.5),
   and the always-on duplicate-frame counter (DEC-0035).
