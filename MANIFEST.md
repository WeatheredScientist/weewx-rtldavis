...
| **`ops/*` + `scripts/*` — the harness**, plus `docs/CAMPAIGN-B-RUNBOOK.md` (swap-night gates, rollback, expected numbers) | any ops task, and the swap night. **Each script's header is its manual — read it before using or extending one**: why it exists, the lying symptom it catches, its gotchas. `ops/campaign_analyze.py` is the **only** sanctioned campaign readout (DEC-0069) |
