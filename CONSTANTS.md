...
| `[DavisPressure]` → `fetch_interval` | **300** (queued S99/DEC-0113, not yet applied — live value is still **3600** as of this writing) | WeatherLink v2's documented ceiling is 1,000 calls/hour + 10/s; 300s uses ~1.2% of quota. Cuts the archived barometer from a 60-min sample-and-hold staircase to a 5-min one (#144 item 3). Takes effect only at the next `weewxd` restart — deferred to the ~08-23 v2.0.14 event alongside everything else behind Campaign B's comparability discipline. **Apply and verify** (`nasctl conf ... DavisPressure` shows `300`) at that event; a stock recreate before then would leave the code default (3600) running, same failure shape as this table's other rows |
