...
| Thing | Value |
|-------|-------|
| Prod image | **`:v2.0.15`** since 2026-09-03 07:17:53 EDT (DEC-0136) — built **on marvin** (`marvinctl build`, self-service) from `origin/dev`@`2fa80a4`, carrying the dupgate patch (DEC-0135). Before it, `:v2.0.14` ran from 2026-08-23 (S101) through the host move (DEC-0118: same bits, `docker save`/`load`, no rebuild); it was NAS-built from `origin/dev`@`efeeebd` under `ops/nas_build.py`'s NAS-LEASE wrapper (DEC-0114) and carries DEC-0110/DEC-0111/#233/#224/DEC-0113 and the weewx 5.4.0→5.5.0 bump. Driver banner `0.20+ws.5` |
| Docker Hub | Still `:v2.0.13` — v2.0.15's Hub push follows prod proof, per DEC-0078 (`BOOT.md` job 3) |
| Rollback (image) | `:v2.0.14` (the prior prod image) was loaded onto marvin at DEC-0118 — presence not re-verified since; `:v2.0.13` on Docker Hub, `:v2.0.12` one step further back |
