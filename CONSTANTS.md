...
| Container | `weewx-rtldavis-v2` (now on marvin) — runs as `t-weewx` (996:986) via `--user` on the unit since DEC-0147 (S126); the published image itself stays root-by-default |
...
| **`weewx_monitor.py` — host-side daemon, NOT in the container** (verified S119; deploy path changed S129) | the **file at `/srv/docker/weewx/weewx_monitor.py`**, run by `weewx-monitor.service` in `/weather.slice` | **A merge no longer "does nothing" — the tenant root became a real `dev` checkout at S129 (DEC-0150, ops#257), so a routine `marvinctl pull` now DOES update this file's on-disk bytes for any commit that touches it.** What still doesn't happen automatically is the **process** picking it up: the daemon reads the file at its own startup only, so a `pull` alone leaves old code running until `marvinctl --tenant weewx restart weewx-monitor.service` (self-service, no owner gesture needed anymore — the old owner-run curl+`sudo install` transport is retired). This makes the deploy a **two-step, both self-service**: `pull`, then a deliberate `restart` once the on-disk change is actually meant to go live — don't restart reflexively just because `pull` touched the file. Unit-file edits (`/etc/systemd/system/weewx-monitor.service`, root:root) are still owner-run + `daemon-reload` — that boundary is unchanged, it's file-content deploy that moved. Verify by comparing on-disk sha to `dev`'s tip, start time after file mtime, and the `Remedy armed:` startup line |
...
| file modes the non-root container needs | `archive/weewx.sdb` owner-writable (was `r-x------`), `logs/weewx.log` owned 996:986 | DEC-0147: a root-written file in the tree is unwritable for the container once it runs as 996; these two are the known ones (the rest are tmp+rename or read-only). A recreate that leaves a root-owned hot `weewx.sdb-journal` blocks the DB open — see DEC-0147 |

## Release / rollback

| Thing | Value |
|-------|-------|
| Prod image | **`:v2.0.16`** since 2026-09-03 20:29:06 EDT (DEC-0138) — built **on marvin** (`marvinctl build`, self-service) from `origin/dev`@`73acc3d`, carrying #317's slot-count denominator (DEC-0137). Tree transport was the same one-off owner-authorized `git archive` → `scp` → `sudo tar` extraction DEC-0136 used — historical fact for this build; **ops#257 closed S129** (DEC-0150), so the next build no longer needs this mechanism, see "Release mechanics" below. Cutover outage **31s** (same-host container recreate, not a rebuild wait). Before it, `:v2.0.15` ran from 2026-09-03 07:17:53 EDT (DEC-0136), carrying the dupgate patch (DEC-0135). Driver banner `0.20+ws.5` unchanged across both · **Runs as `t-weewx` (996:986) since 2026-09-06 13:12:05 EDT** (DEC-0147, a unit `--user` flag — image bytes unchanged, MARVIN-DEC-0140) |
| Docker Hub | **`:v2.0.16` since 2026-09-04 11:30 ET**, pushed by **S122 as ops#265's one-off stopgap, DEC-0078's own pattern:** owner-run `docker save` as root on marvin (`ssh marvin-sudo`, Touch-ID-gated — a raw `sudo -v` over ssh hung first, ops#266) → tarball fetched to the laptop → `docker load` → `docker push` **from the laptop**, which holds the Hub login. Not via `marvinctl` (its `push` verb is not installed yet). S122 verified the digest but recorded none of this in BOOT/CHANGELOG (S123 backfilled). **Verified byte-identical to prod:** Hub index digest `sha256:1a9daeb6…` = marvin's container `Image` id; amd64 manifest `sha256:badd5056…` = its `ImageManifestDescriptor`. Marvin runs the **containerd image store**, so its image IDs are index digests — compare index-to-index, never config-digest-to-`Image`, or an identical image looks like a mismatch (S123). **`:latest` still = `:v2.0.13`** (`4cfc7fb9…`, 2026-08-21) — moving it is the owner's Class C gesture after prod proof (DEC-0078; proof exists, DEC-0139). `:v2.0.14`/`:v2.0.15` never pushed. Check Hub, not this row: `curl -s https://hub.docker.com/v2/repositories/weatheredscientist/weewx-rtldavis/tags` |
| Rollback (image) | `:v2.0.15` is still present on marvin (the prior prod image, `build-v2.0.15/` + tagged image, not removed at this cutover); `:v2.0.14` was loaded at DEC-0118 — presence not re-verified since; `:v2.0.13` on Docker Hub, `:v2.0.12` one step further back · the DEC-0147 user change is a unit-flag revert (marvin gesture), not a retag — the image bytes did not change |
...
| Release mechanics | **Self-service since 2026-09-07 (S129, DEC-0150, ops#257 limb 1).** `/srv/docker/weewx` is now a real `dev` checkout (`origin` = the SSH deploy-key URL, `MARVIN-DEC-0144`) — `marvinctl --tenant weewx pull` fast-forwards it, verified live end-to-end (not just CoffeeRadar's unproven precedent). Build/deploy is still `marvinctl build <path> -t <tag>` → `marvinctl set-image` → `marvinctl restart weewx.service`, `kill` → `rm` → `run` semantics unchanged (never `compose up`; `docker-compose.yml` remains decorative). The one thing `pull` does **not** safely cover yet: `weewx_monitor.py` is git-tracked at the tenant root but runs as a host daemon read directly off disk (`weewx-monitor.service`, not rebuilt) — a routine `pull` **will** update this file's on-disk bytes for any future merge that touches it, but the running process won't see the change until `weewx-monitor.service` is explicitly restarted. **Reconciled same session (S129):** diffed the pre-swap deployed copy against `dev`'s tip — comment/docstring text plus one email-summary string correcting stale #317 wording, zero functional change — adopted `dev`'s tip and restarted `weewx-monitor.service`; sha now matches `dev` exactly. Old deployed copy kept as `weewx_monitor.py.pre-reconcile-20260907`, not deleted |
