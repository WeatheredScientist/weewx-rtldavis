...
def main():
    now = time.time()
    last_seen   = {s: 0.0 for s in THRESHOLDS}  # 0 = not yet seen; countdown starts after first post
    ...
    while True:
        ...
        if cur < last_line:
            ...
            last_line = 0
            # Reset last_seen to 0 after log rotation — countdown restarts after first post
        ...
        for svc, thr in THRESHOLDS.items():
            ...
            if last_seen[svc] == 0.0:
                continue  # skip until first post seen
            if age > thr:
