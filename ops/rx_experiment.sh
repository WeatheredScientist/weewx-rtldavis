...
# Generated by: ops/rx_experiment.sh schedule --generate <YYYY-MM-DD>
...
send_mail() {
  ...
  [ -f "$ENVFILE" ] || { log "MAIL SKIPPED (no $ENVFILE)"; return 1; }
...
health_ok() {
  ...
  for i in $(seq 1 18); do          # up to ~90s
    sleep 5
